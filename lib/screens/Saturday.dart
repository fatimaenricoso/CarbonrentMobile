...
class Saturday extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _buildSaturdayVendorList(context),
    );
  }

  Widget _buildSaturdayVendorList(BuildContext context) {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return const Center(child: Text('No vendors assigned on Saturday'));
        }

        final vendors = snapshot.data!.docs;

        final saturdayVendors = vendors.where((vendor) {
          // Check for vendors assigned on Saturday
          return _isVendorAssignedOnDay(vendor, 'Saturday');
        }).toList();

        if (saturdayVendors.isEmpty) {
          return const Center(child: Text('No vendors assigned on Saturday'));
        }

        return ListView.builder(
          itemCount: saturdayVendors.length,
          itemBuilder: (context, index) {
            final vendor = saturdayVendors[index];
            final firstName = vendor['first_name'];
            final lastName = vendor['last_name'];
            final vendorId = vendor.id; // Get the vendor ID for deletion

            // Find the specific field that contains 'Saturday'
            String? saturdayField = _findSaturdayField(vendor);
                ...
                child: Row(
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        if (saturdayField != null) {
                          _deleteVendorAssignment(vendorId, saturdayField, context);
                        } else {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('No Saturday assignment found')),
                          );
                        }
                      },
                    ),
  ...
  String? _findSaturdayField(QueryDocumentSnapshot vendor) {
    final vendorData = vendor.data() as Map<String, dynamic>?;
    ...
    for (int i = 1; i <= 10; i++) {
      final fieldValue = vendorData['day_assign_$i'];
      if (fieldValue == 'Saturday') {
        return 'day_assign_$i'; // Return the specific field name
      }
    }
    return null; // No Saturday assignment found
  }
