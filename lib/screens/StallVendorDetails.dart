import 'package:ambulantcollector/screens/StallReceiptPaid.dart';
...
class _StallDetailsState extends State<StallDetails> {
  ...
  Future<void> _markAsPaid(List<DocumentSnapshot> payments) async {
    ...
    if (shouldProceed) {
      List<Map<String, dynamic>> markedPayments = [];

      for (var payment in payments) {
        var paymentData = payment.data() as Map<String, dynamic>;
        markedPayments.add(paymentData);
      ...
      _showPaymentMarkedDialog(markedPayments);
    }
  }
  ...
  Future<bool> showConfirmationDialog(BuildContext context, List<DocumentSnapshot> payments) async {
    num totalAmount = payments.fold<num>(0.0, (num sum, payment) {
      var paymentData = payment.data() as Map<String, dynamic>;
      if (paymentData.containsKey('totalAmountDue')) {
  ...
  void _showPaymentMarkedDialog(List<Map<String, dynamic>> markedPayments) {
    showDialog(
      context: context,
                  ...
                  TextButton(
                    onPressed: () {
                      ...
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => StallReceiptPaid(
                            payments: markedPayments,
                            vendorFullName: '${widget.vendor['firstName']} ${widget.vendor['middleName']} ${widget.vendor['lastName']}',
                          ),
                        ),
                      );
                    },
  ...
  Widget _buildInfoColumnWithViewLink(IconData icon, String label, String value, {String? subtitle, Color? iconColor, DocumentSnapshot? payment, VoidCallback? onViewTap, required String amount}) {
    var paymentData = payment?.data() as Map<String, dynamic>?;
    String amount = paymentData?.containsKey('totalAmountDue') == true
        ? 'Amount: ₱${paymentData?['totalAmountDue'] ?? 0}'
