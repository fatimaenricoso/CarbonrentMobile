...
class Sunday extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _buildSundayVendorList(context),
    );
  }

  Widget _buildSundayVendorList(BuildContext context) {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return const Center(child: Text('No vendors assigned on Sunday'));
        }

        final vendors = snapshot.data!.docs;

        final sundayVendors = vendors.where((vendor) {
          // Check for vendors assigned on Sunday
          return _isVendorAssignedOnDay(vendor, 'Sunday');
        }).toList();

        if (sundayVendors.isEmpty) {
          return const Center(child: Text('No vendors assigned on Sunday'));
        }

        return ListView.builder(
          itemCount: sundayVendors.length,
          itemBuilder: (context, index) {
            final vendor = sundayVendors[index];
            final firstName = vendor['first_name'];
            final lastName = vendor['last_name'];
            final vendorId = vendor.id; // Get the vendor ID for deletion

            // Find the specific field that contains 'Sunday'
            String? sundayField = _findSundayField(vendor);
                ...
                child: Row(
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        if (sundayField != null) {
                          _deleteVendorAssignment(vendorId, sundayField, context);
                        } else {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('No Sunday assignment found')),
                          );
                        }
                      },
                    ),
  ...
  String? _findSundayField(QueryDocumentSnapshot vendor) {
    final vendorData = vendor.data() as Map<String, dynamic>?;
    ...
    for (int i = 1; i <= 10; i++) {
      final fieldValue = vendorData['day_assign_$i'];
      if (fieldValue == 'Sunday') {
        return 'day_assign_$i'; // Return the specific field name
      }
    }
    return null; // No Sunday assignment found
  }
