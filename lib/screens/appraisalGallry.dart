...
class AppraisalGallery extends StatefulWidget {
  const AppraisalGallery({Key? key}) : super(key: key);

  @override
  _AppraisalGalleryState createState() => _AppraisalGalleryState();
}

class _AppraisalGalleryState extends State<AppraisalGallery> {
  ...
  Future<void> _confirmDeleteImage(int index) async {
    ...
    if (user != null) {
      ...
      if (userQuery.docs.isNotEmpty) {
        ...
        // Dismiss the image preview dialog if it is still open
        if (Navigator.canPop(context)) {
          Navigator.of(context).pop();
        }

        // Fetch the updated profile images
        _getProfileImages();
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('User not found.')),
